...
class Tank constructor(
    ...
    private fun getTankNextCoordinate(view: View): Coordinate {
        ...
        when (direction) {
            ...
            Direction.DOWN -> {
                (view.layoutParams as FrameLayout.LayoutParams).topMargin += CELL_SIZE  // Исправлено
            }
            Direction.LEFT -> {
                (view.layoutParams as FrameLayout.LayoutParams).leftMargin -= CELL_SIZE
            }
            Direction.RIGHT -> {
                (view.layoutParams as FrameLayout.LayoutParams).leftMargin += CELL_SIZE  // Исправлено
            }
        }
